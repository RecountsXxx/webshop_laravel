...
class VendorUsersService {
    toast = useToast();

    async fetchUsers(pageNumber = 1) {
        try {
            const response = await axios.get(`http://localhost/api/admin/vendor-users/?page=${pageNumber}`);
    ...
    async addUser(formData) {
        try {
            const response = await axios.post('http://localhost/api/admin/vendor-users', formData);
    ...
    async getUser(id) {
        try {
            const response = await axios.get(`http://localhost/api/admin/vendor-users/${id}`);
            return response.data.data.user;
        } catch (error) {
    ...
    async editUser(formData, id) {
        try {
            await axios.put(`http://localhost/api/admin/vendor-users/${id}`, formData);
    ...
    async deleteUser(userId) {
        try {
            await axios.delete(`http://localhost/api/admin/vendor-users/${userId}`);
...
export default new VendorUsersService();
