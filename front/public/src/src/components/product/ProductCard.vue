    ...
    <div class="product__item sale">
      ...
      <div class="product__item__text">
        ...
        <div class="rating flex flex-row justify-between">
          ...
          <div class="ms-5">
            <h5 :class="{ 'text-secondary text-sm font-sans': count > 0, 'text-secondary text-sm font-sans': count <= 0 }">{{ count > 0 ? 'In stock' : 'Ended' }}</h5>
          </div>
        </div>
...
export default {
  ...
  methods: {
    clickBookmark() {
      ...
      if (this.isLiked) {
        ProductService.addToBookmark(this.id, this.title, this.new_price || this.price, this.image);
      } else {
    ...
    clickOrder() {
      if (parseInt(this.count) > 0) {
        ProductService.addToCart(this.id, this.title, this.new_price || this.price, this.image, this.vendor_id);
      } else {
        this.$notify({
          title: "Products his not in stock 🎉",
          type: 'error'
        });
      }
    }
